    ...
    while(filepos<NR_DENTRY_IN_ZONE){
        if(test_bit_le(bitpos, (void *)z_p->statemap)){
            ...
            if(de->isr_sf==0 ){
                ...
                if(par_pos==ppos) {
                    fp = kzalloc(sizeof(struct file_p), GFP_KERNEL);
                    fp->pos = filepos;
    ...
    while(filepos<NR_DENTRY_IN_ZONE){
        if(test_bit_le(bitpos, (void *)zp->statemap)){
            ...
            if(de->file_type==NORMAL_DIRECTORY){
                ...
                dir_i = kzalloc(sizeof(struct dir_info), GFP_KERNEL);
                dir_i->r_f = 0;
                dir_i->sub_num = 0;
                dir_i->sub_s =0;
                dir_i->f_s = 0;
...
struct dzt_entry_info *add_dzt_entry(struct super_block *sb, struct dzt_entry_info *par_dei, u32 sp_id)
{
    ...
    if(par_dei->dzt_eno!=0){
        /*not decided*/
        cur_name = kzalloc(sizeof(char)*(de_nlen+1), GFP_KERNEL);
        /*get ful_name of rde*/
        ...
        int par_pos, struct dir_info *par_f, u32 ch_no, u32 *ch_pos, int isr_sf)
{
    ...
    name = kzalloc(sizeof(char)*LARGE_NAME_LEN, GFP_KERNEL);
    //BUG_ON(old_de==ROOT_DIRECTORY);
    ...
    //BUG_ON(old_de->file_type==ROOT_DIRECTORY);
    if(isr_sf==0)
    ...
    if(old_de->file_type == NORMAL_FILE){
        ...
        tname = kzalloc(sizeof(char)*name_len, GFP_KERNEL);
        /*get fulname*/
        /*
        ...
        *ch_pos = new_id;
        ...
        name_len = le64_to_cpu(old_de->fname_len) - old_len;
        new_de->fname_len = cpu_to_le64(name_len);
        fname = kzalloc(sizeof(char)*(name_len+1), GFP_KERNEL);
        tname = kzalloc(sizeof(char)*(name_len+1), GFP_KERNEL);
        ...
        * set ful_name*/
        ...
        //name_len = le64_to_cpu(old_de->fname_len);
        hashname = BKDRHash(tname, name_len);
        make_invalid_htable(sb, old_ei->ht_head, hashname, 1);
...
int migrate_zone_entry(struct super_block *sb, u32 ch_pos, struct dzt_entry_info *dzt_nei)
{
    ...
    * not change rde sub_state*/
    ...
    //nova_flush_buffer(dafs_rde, DAFS_DEF_DENTRY_SIZE, 0);
                    ...
                    struct zone_ptr *z_p, u32 start_id)
{
    ...
    u32 inv_id = 0;
    //u32 eno;
    while(inv_id<NR_DENTRY_IN_ZONE){
        if(test_bit_le(bitpos, (void *)z_p->statemap)){
            bitpos+=2;
            inv_id++;
            continue;
...
/*merge
* new_sp merge pos*/
int __merge_dentry(struct super_block *sb, struct dzt_entry_info *cur_ei, u32 cur_pos,\
                  u32 rde_pos, u32 *new_sp)
{
    ...
    rname = kzalloc(sizeof(char)*(rnamelen+1), GFP_KERNEL);
    ...
    if(cur_de->file_type != NORMAL_DIRECTORY){
        ...
        /*set name and ext_flag*/
        name = kzalloc((cur_de->name_len)*sizeof(char), GFP_KERNEL);
        ...
        /*set fulname and dzt_hn or hname
        * set root dir info*/
        if(des_de->file_type == NORMAL_FILE){
            ...
            new_idir = add_dir_info(par_ei, hn, fpos);
            o_plen = le64_to_cpu(cur_de->fname_len);
            ...
            hn = BKDRHash(tname, o_plen);
            old_idir = radix_tree_delete(&cur_ei->dir_tree, hn);
            head = &old_idir->sub_file;
            list_for_each_safe(this, next, head){
                old_sf = list_entry(this, struct file_p, list);   
                new_sf = kzalloc(sizeof(struct file_p), GFP_KERNEL);
                new_sf->pos = old_sf->pos;
                list_add_tail(&new_sf->list, &new_idir->sub_file);
                //new_idir->sub_num++;
                old_idir->sub_num--;
                kfree(old_sf);
            }
        ...
        *new_sp = fpos;
        ...
        list_for_each(this, head){
            ...
            pos = o_sub->pos;
            ret = __merge_dentry(sb, cur_ei, pos, fpos, &tem_pos);
            fp = kzalloc(sizeof(struct file_p),GFP_KERNEL);
            fp->pos = tem_pos;
            list_add_tail(&fp->list, &new_idir->sub_file);
            //new_idir->sub_num++;
        }
    }
    kfree(rname);
...
int merge_dentry(struct super_block *sb, struct dzt_entry_info *cur_ei)
{
    ...
    u32 bitpos, filepos = 0, rde_pos, tem_pos;
    u64 dir_hn;
    int ret = 0;
    ...
    if(rde->file_type!=ROOT_DIRECTORY){
        BUG();
        return -EINVAL;
    }
   
    dir_hn = le64_to_cpu(rde->hname);
    ...
    while(bitpos<cur_p->zone_max){
        if(test_bit_le(bitpos, (void *)cur_p->statemap)){
            ...
            if(de->isr_sf==1){
                __merge_dentry(sb, cur_ei, filepos, rde_pos,&tem_pos);
            }
            /*
                  ...
                  u32 rde_pos, u32 *new_sp)
{
    ...
    struct file_p *fp, *o_sub, *old_sf, *new_sf; 
    struct list_head *this, *head, *next;
    u32 bitpos = 0, fpos = 0, eno, pos, tem_pos;
    ...
    if(cur_de->file_type == NORMAL_FILE || cur_de->file_type == ROOT_DIRECTORY){
        ...
        * delete in hashtable*/
        if(des_de->file_type == NORMAL_FILE){
            ...
            new_idir = add_dir_info(par_ei, hn, fpos);
            /*delete in previous hashtable*/
            pname = kzalloc(sizeof(char)*plen,GFP_KERNEL);
        ...
        *new_sp = fpos;
        ...
        list_for_each(this, head){
            o_sub = list_entry(this, struct file_p, list);
            pos = o_sub->pos;
            ret = __inherit_dentry(sb, cur_ei, pos, fpos, &tem_pos);
            fp = kzalloc(sizeof(struct file_p),GFP_KERNEL);
            fp->pos = tem_pos;
            list_add_tail(&fp->list, &new_idir->sub_file);
        }
        ...
        if(par_ei->dzt_eno !=1){
            ...
            pname = kzalloc(sizeof(char)*(zlen+1), GFP_KERNEL);
...
void inherit_dentry(struct super_block *sb, struct dzt_entry_info *cur_ei)
{
    ...
    while(bitpos<cur_p->zone_max){
        if(test_bit_le(bitpos, (void *)cur_p->statemap)) {
            de = &cur_ze->dentry[filepos];
            if(de->isr_sf==1)
                __inherit_dentry(sb, cur_ei, filepos, rde_pos, &new_sp);
            bitpos++;
        } else {
            bitpos++;
            if(test_bit_le(bitpos, (void *)cur_p->statemap)){
...
int dafs_merge_zone(struct super_block *sb, struct dzt_entry_info *cur_rdei)
{
    ...
    struct dzt_ptr *dzt_p;
    u32 ch_pos, or_pos, eno, sub_pos, new_sp;
    struct dir_info *or_dir=NULL;
    ...
    * delete dzt on nvm*/
    ...
    rname = kzalloc(sizeof(char)*(rlen+1), GFP_KERNEL);
    ...
    or_dir = radix_tree_lookup(&par_ei->dir_tree, root_hash);
    BUG_ON(or_dir==NULL);

    head = &or_dir->sub_file;
    list_for_each(this, head){
        tem_sf = list_entry(this, struct file_p, list);
        sub_pos = tem_sf->pos;
        __merge_dentry(sb, cur_rdei, sub_pos, or_pos, &new_sp);
        /*update pos*/
        tem_sf->pos = new_sp;
        or_dir->sub_num++;
    }
...
int dafs_inh_zone(struct super_block *sb, struct dzt_entry_info *cur_rdei, u32 nr_pos)
{
    ...
    list_for_each(this, head){
        tem_sf = list_entry(this, struct file_p, list);
        sub_pos = tem_sf->pos;
        __inherit_dentry(sb, cur_rdei, sub_pos, or_pos, &new_sp);
        /*update pos*/
        tem_sf->pos = new_sp;
        or_dir->sub_num++;
    }
