...
END:
    nova_dbg("dafs finish finding dzt");
    return dzt_ei;
}
...
int dafs_add_dentry(struct dentry *dentry, u64 ino, int inc_link, int file_type)
{
    ...
    unsigned long phlen, temlen, flen;
    ...
    * use temlen to find par de*/
    ...
    kfree(phn);
    kfree(zone_p);
    nova_dbg("dafs finish add dentry");
    return ret;
}
...
struct dafs_dentry *dafs_find_direntry(struct super_block *sb, const struct dentry *dentry, int update_flag)
{
    ...
    nova_dbg("dafs start to find direntry");
    ph = get_dentry_path(dentry);
    ...
    kfree(phn);
OUT:
    nova_dbg("dafs finish find direntry");
    return direntry;
}
        ...
        struct dafs_zone_entry *dafs_ze, struct dzt_entry_info *dzt_ei, u32 de_pos)
{
    ...
    //char *par_name, *tem;
    u64 hashname, d_hn, tail, par_hn, plen;
    char *pname;
    int ret, isr_sf;
    //u8 hlevel = 1;
        ...
        struct dzt_entry_info *dzt_ei, const char *path, char *name)
{
    struct nova_sb_info *sbi = NOVA_SB(sb);
    struct zone_ptr *z_p;
    struct dafs_dentry *new_de, *sub_de;
    struct dafs_zone_entry *ze;
...
int add_rename_dir(struct dentry *o_dentry, struct dentry *n_dentry, struct dafs_dentry *old_de)
{
   ...
   nova_dbg("dafs add renmae dir"); 
    ...
    ret = __rename_dir(sb, old_de, n_ei, phn, n_name);
...
int __rename_file_dentry(struct dentry *old_dentry, struct dentry *new_dentry)
{
    ...
    temlen = phlen-new_dentry->d_name.len;
    if(temlen == 1){
        dafs_de->isr_sf = 1;
        dafs_de->par_pos = 0;
    } else {
