...
static inline char* get_dentry_path(struct dentry *dentry)
{
    ...
    buf = kzalloc(sizeof(char)*LARGE_NAME_LEN,GFP_KERNEL);
    if(!buf)
        goto ERR;
    ...
    path.mnt = vfsmnt;
    path.dentry = dentry;
    ph = d_path(&path, buf, LARGE_NAME_LEN);
    mntput(vfsmnt);
...

{
    ...
    u64 ext_len;
    u32 bitpos, ext_pos, eno, ext_num;
...
int dafs_add_dentry(struct dentry *dentry, u64 ino, int inc_link, int file_type)
{
    ...
    * use temlen to find par de*/
    ...
    if(file_type==1){
        ...
        dafs_append_dir_init_entries(sb, par_pos, dzt_ei, ino, dir->i_ino, phn);
    } else {
        ...
        struct dafs_zone_entry *dafs_ze, struct dzt_entry_info *dzt_ei, unsigned long de_pos)
{
    ...
    char *par_name, *tem;
    u64 hashname, d_hn, d_hlen, tail, par_hn;
    int ret, isr_sf;
    u8 hlevel = 1;
                                ...
                                u64 self_ino, u64 parent_ino, const char *ful_name)
{
    ...
    */
    ...
    if(cur_pos == NR_DENTRY_IN_ZONE){
        dafs_split_zone(sb, NULL, 0, dzt_ei);
    }
