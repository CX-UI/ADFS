...
static inline char* get_dentry_path(const struct dentry *dentry, int ISREAD)
{
    ...
    ph = kzalloc(sizeof(char)*DAFS_PATH_LEN, GFP_ATOMIC);
    buf = kzalloc(sizeof(char)*DAFS_PATH_LEN, GFP_ATOMIC);
    tem = kzalloc(sizeof(char)*DAFS_PATH_LEN, GFP_ATOMIC);
    if(!buf)
        goto ERR;
    ...
    //BUG_ON(strcmp(p_dentry->d_name.name, "/"));
    ...
    //memcpy(tem, rd->d_name.name, rd->d_name.len);
    do{
        strcat(buf, "/");
        strcat(buf, tem_dentry->d_name.name);
        //memcpy(ph, buf, strlen(buf));
    ...
    nova_dbg("%s dentry name is %s",__func__, (path.dentry)->d_name.name);
        ...
        u64 *new_hn, u64 *root_len)
{
    ...
    tname = kzalloc(sizeof(char)*phlen,GFP_ATOMIC);
    ...
    list_for_each_safe(this, next, head) {
        ...
        if(sub_de->file_type == NORMAL_DIRECTORY){
            ...
            if(new_de->file_type==ROOT_DIRECTORY){
                ...
                r_name = kzalloc(sizeof(char)*(rn_len+1),GFP_ATOMIC);
...
static int dafs_readdir(struct file *file, struct dir_context *ctx)
{
    ...
    /*debug*/
    //ppath = kzalloc(sizeof(char)*SMALL_NAME_LEN,GFP_ATOMIC);
